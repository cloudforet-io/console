...
<Canvas>
    <Story name="Multi Selectable" args={{menu: menuItems}} height={'20rem'}>
        {{
            ...
            template: `
        <div>
            <p-context-menu :menu="menu" :multi-selectable="true" :selected.sync="selected" />
            <br/>
...
|-|-|-|
|Additional space|A slot that located at the top of the Context Menu.|`top`|
...
<Canvas>
    <Story name="Top Slot" args={{menu: menuItems}}>
        {{
            props: Object.keys(getContextMenuArgTypes()),
            components: { PContextMenu, PRawData },
            template: `
        <div class="flex h-full flex-col">
            <div class="mt-4">
                <h2 class="mb-4">This is <strong>top</strong> slot</h2>
                <p-context-menu :menu="menu">
                    <template #top>
                        Top Slot: used in PSearchDropdown!
                    </template>
                </p-context-menu>
            </div>
            <div class="mt-4">
