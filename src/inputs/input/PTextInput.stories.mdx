...
<Meta title='Inputs/InputNew' parameters={{
    design: {
        type: 'figma',
...
## Search
<Canvas>
    <Story name="AutoComplete" args={{menu: getSearchDropdownMenu()}}>
        {{
            props: Object.keys(getSearchDropdownArgTypes()),
            components: { PTextInput },
            template: `
                <p-text-input :menu="menu"  multi-input use-fixed-menu-style auto-complete />
            `,
            setup() {
                const state = reactive({
                })
...
<Canvas>
    <Story name="Block">
        {{
            components: { PTextInput },
            template: `
                <p-text-input :entered="entered" block multi-input />
            `,
            setup() {
                const state = reactive({
                    entered: [{
...
<Canvas>
    <Story name="Right Extra Slot" args={{
        ...
        {{
            components: { PTextInput },
            template: `
                <p-text-input style="width: 80px;" multi-input v-model="value" >
