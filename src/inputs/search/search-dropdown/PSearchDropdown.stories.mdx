...
<Canvas>
    <Story name="Using Handler" >
        {{
            components: { PSearchDropdown, PButton },
            template: `
            <p-search-dropdown :menu="menu" :loading="loading" :handler="simpleHandler" use-fixed-menu-style />
`,
            setup() {
                const state = reactive({
                    loading: false,
                    menu: getSearchDropdownMenuWithMultiTypes()
                })
                const simpleHandler = async (inputText, list) => {
                    state.loading = true;
