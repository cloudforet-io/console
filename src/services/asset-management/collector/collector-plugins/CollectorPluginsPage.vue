...
import PluginFilter from '@/services/asset-management/collector/collector-plugins/modules/PluginFilter.vue';
...
export default {
    ...
    setup() {
        ...
        const onPluginCreate = (item) => {
            vm.$router.push({ name: ASSET_MANAGEMENT_ROUTE.COLLECTOR.CREATE.STEPS._NAME, params: { pluginId: item.plugin_id } });
        };
        const onSearch = (val) => {
            state.keyword = val;
            getPlugins();
