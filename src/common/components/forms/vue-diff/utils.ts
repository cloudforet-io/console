...
const getSplitLines = (diffsMap: Array<Diffs>): Array<Lines> => {
    ...
    diffsMap.forEach((diffs) => {
        ...
        for (let i = 0; i < loopCount; i++) {
            const hasPrevLine = getDiffType(diffs[0][0]) !== 'disabled'
        && typeof prevLines[i] !== 'undefined';
            const hasCurrentLine = getDiffType(diffs[1][0]) !== 'disabled'
        && typeof currentLines[i] !== 'undefined';

            if (hasPrevLine) lineNum.prev += 1;
            if (hasCurrentLine) lineNum.current += 1;
...
const renderLines = (
    ...
    function diffLines(_prev: string, _current: string) {
        const dmp = new DiffMatchPatch();
        const a = dmp.diff_linesToChars_(_prev, _current);
   ...
   */
    const diffsMap = diffLines(prev, current).reduce(
        (acc: Array<Diffs>, curr) => {
            ...
            if (type === 'added') {
                const _prev = acc.length && acc[acc.length - 1][0] ? acc[acc.length - 1][0] : null;
        ...
        node.childNodes.forEach((child) => {
            ...
            if (child.nodeType === 3) {
                ...
                while (oldContent.length) {
                    ...
                    const originalCodeDiffLength = hasModifiedTag && hasModifiedTag.index
                    ...
                    newContent += originalCode.substring(0, nextDiffsLength);
