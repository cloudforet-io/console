...
import PIconTextButton from '@/inputs/buttons/icon-text-button/PIconTextButton';
...
<Canvas>
    <Story name="Customize Loader">
        {{
            components: { PListCard, PIconTextButton },
            template: `
    <div class="h-full w-full overflow p-8" style="height: 200px;">
         <p-icon-text-button
                class="mb-4" name="ic_refresh"
                @click="updateItems"
        >
             Refresh
        </p-icon-text-button>
    ...
    `,
            setup() {
                ...
                const updateItems = () => {
                    state.loading = true;
                    setTimeout(() => {
                        state.items =range(faker.random.number({ min: 0, max: 3 }))
