...
export default {
    ...
    computed: {
        ...
        getGreetMessage() {
            ...
            return !this.isEmpty(companyTitle) ? this.$t('SIGNIN.WELCOME_MSG', [companyDesc]) : !this.isEmpty(companyTitle) ? this.$t('SIGNIN.WELCOME_MSG_P', [companyTitle]) : this.$t('SIGNIN.WELCOME_MSG_P', [hostName]);
        },
        getCurrentHostname() {
            const companyTitle = this.$store.getters['domain/companyTitle'];
            const hostName = this.getWindowHostName();
