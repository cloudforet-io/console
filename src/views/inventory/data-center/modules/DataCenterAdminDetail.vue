<template>
  <div class="animated fadeIn">
    <b-row>
      <b-col cols="12">
        <BaseTable :table-data="dataCenterAdminData.members"
                   :fields="fields"
                   :per-page="perPage"
                   searchable
                   ...
                   @list="listAdminsOnModal"
                   @limitChanged="limitChanged"
                   @onSelectAll="rowAllSelected"
                   @empty="dataCenterAdminData.members = []"
        />
        <br>
        <b-card>
          <b-col ref="IDPJ006_tagPanel" cols="12" class="row-scroll p-0">
            <InputTag v-for="(tag, idx) in dataCenterAdminData.tagList"
                      ref="tag"
                      :key="tag.id"
                      use-tag-only
                      class="input-tag"
      ...
      <slot name="footerArea">
        <div class="col-md-12">
          <div class="modal-footer" style="border-top:none; padding-right: 0px">
            <b-button size="md" variant="primary" @click="addAdmin">
              {{ $t('MSG.BTN_ADD') }}
            </b-button>
...
import query from '@/views/identity/project/search-context/query';
import BaseTable from '@/components/base/table/BaseTable';
import InputTag from '@/components/base/input/InputTag';

export default {
    name: 'DataCenterAdminDetail',
    events: ['close'],
    components: {
        BaseTable,
        InputTag
    ...
    data() {
        return {
            dataCenterAdminData:{ members:[], tagList: []} ,
    ...
    methods: {
        init() {
            this.listAdminsOnModal(this.perPage, 0);
        },
        reset () {
            this.dataCenterAdminData.members = [];
        ...
        saveMemberModalMeta (limit, start, sort, filter, filterOr) {
            ...
            this.memberModalQuery = {
                ...
                filter_or: filterOr,
                filter: [
                    { key: 'user_id' ,value: this.$attrs.admins, operator: 'not_in' }
                ]
            };
        },
        ...
        rowSelected (rows) {
            ...
            const el = this.dataCenterAdminData.tagList.filter(function(el) {
                return el.key === rows[0].data.user_id;
            });

            if (!el.length){
                this.dataCenterAdminData.tagList.push(selectedObject);
                console.log('tagValue', this.dataCenterAdminData.tagList);
        ...
        async addAdmin() {
            ...
            const url = `/inventory/${selected_type.toLowerCase()}/admin/add`;
            ...
            if (this.selectedModalItems.length > 0){
                const selectedUsers = this.dataCenterAdminData.tagList;
                param['users'] = this.getSelectedValArr(selectedUsers,'key');
            } else {
                return;
            }
        ...
        deleteAdmin (idx) {
            this.$delete(this.dataCenterAdminData.tagList, idx);
        },
        onfocusOnBottom () {
