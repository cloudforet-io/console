...
<script lang="ts">
import { map, every, debounce } from 'lodash';
import {
    reactive, toRefs, getCurrentInstance, computed,
} from '@vue/composition-api';
import { DataTableState } from '@/components/organisms/tables/data-table/PDataTable.toolset';
import { makeTrItems } from '@/lib/view-helper';
import { dictPanelProps, DictPanelPropsType } from '@/views/common/tags/dict-panel/PDictPanel.toolset';
    ...
    setup(props: DictPanelPropsType, { emit }) {
        ...
        const state: any = reactive({
            ...
            tableState: new DataTableState({
                ...
                items: computed(() => map(props.dict, (v, k) => ({ name: k, value: v }))),
                colCopy: true,
            }).state,
        ...
        const isAllValid = computed(() => every(invalidMessages.value, (item: any) => !item.key && !item.value));
