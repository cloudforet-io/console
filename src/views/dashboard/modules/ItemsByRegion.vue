...
export default {
    ...
    methods: {
        ...
        async listItems() {
            try {
                const res = await this.$http.post('/statistics/datacenter-items', this.getParams());
                this.items = res.data;
        ...
        getBubbleData(count, idx) {
            const x = this.getRandomArbitrary(this.minX, this.maxX);
            const y = this.getRandomArbitrary(this.minY, this.maxY);
            const r = count * this.radius;
            this.setMinMax(r);
            return {
                x, y, r, value: count,
            };
