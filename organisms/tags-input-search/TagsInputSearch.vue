<template>
    <!-- copy template source code from VueTagsInput Component -->
    <div
        class="vue-tags-input"
        :class="[{ 'ti-disabled': disabled }, { 'ti-focus': focused }]"
    >
        <div class="ti-input" ref="inputBox">
            <ul v-if="tagsCopy" class="ti-tags">
                ...
                <li class="ti-new-tag-input-wrapper" ref="inputWrapper">
                    <input
                        ref="newTagInput"
                        v-bind="$attrs"
...
export default {
    ...
    data() {
        return {
            autocompleteMargin: { 'margin-left': '0' },
        };
    },
    watch: {
        tags() {
    ...
    methods: {
        setMargin() {
            const boxLeft = this.$refs.inputBox.getBoundingClientRect().left;
            const inputLeft = this.$refs.inputWrapper.getBoundingClientRect().left;
            this.autocompleteMargin = { 'margin-left': `${inputLeft - boxLeft}px` };
        },
    },
};
</script>
