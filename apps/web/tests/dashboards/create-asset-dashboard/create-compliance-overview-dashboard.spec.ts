...
test.describe('Create Compliance Overview Dashboard', () => {
    test('Create entire Workspace dashboard', async ({ page }) => {
        ...
        await test.step('3. Select Template (Compliance Overview)', async () => {
            ...
            await page.locator('.default-dashboard-board .p-board-item').nth(3).click();
        ...
        await test.step('4. Enter and create a dashboard name', async () => {
            ...
            await page.getByPlaceholder(selectedTemplateName).fill(`${'Compliance Overview'}-${dashboardName}`);
