...
<template>
    <section class="provider-add-page">
        ...
        <!--TODO: translation-->
        <p-button-modal
            v-model:visible="state.visibleInstallModal"
            :header-title="t('Would you like to install', {provider: state.selectedProvider})"
            size="sm"
            @confirm="handleConfirmInstallProvider"
        />
        <!--TODO: translation-->
        <p-icon-modal
            v-model:visible="state.visibleResultModal"
            icon-name="ic_check-circle"
            :header-title="t('has been successfully added.', {provider: state.selectedProvider})"
            :button-text="t('Done')"
            size="md"
        />
    </section>
