...
export default {
    ...
    setup(props, { emit }) {
        ...
        const state = reactive({
            ...
            selectedType: currentRoute?.query?.policy_type ?? POLICY_TYPE.MANAGED as PolicyType,
            fields: [
                { name: 'name', label: 'Name' },
            ...
            selectedIdMap: {} as Record<string, PolicyType>,
        ...
        const handleUpdateSelectIndex = (selectedIndices: number[]) => {
            const selectedIdMap: Record<string, PolicyType> = {};
            if (state.selectedType !== POLICY_TYPE.ALL) {
                ...
                Object.entries(state.selectedIdMap).forEach(([id, policyType]) => {
                    if (policyType !== currentPolicyType) {
                        selectedIdMap[id] = policyType as PolicyType;
                    }
                });
            }
