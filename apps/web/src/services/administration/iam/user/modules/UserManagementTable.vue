...
const state = reactive({
    ...
    dropdownMenu: computed(() => ([
        ...
        {
            type: 'item', name: 'delete', label: t('IDENTITY.USER.MAIN.DELETE'), disabled: !state.isSelected,
        },
        { type: 'divider' },
        {
            type: 'item', name: 'enable', label: t('IDENTITY.USER.MAIN.ENABLE'), disabled: !state.isSelected,
        },
        {
            type: 'item', name: 'disable', label: t('IDENTITY.USER.MAIN.DISABLE'), disabled: !state.isSelected,
        },
    ] as MenuItem[])),
...
const clickAdd = () => {
    ...
    userFormState.headerTitle = t('IDENTITY.USER.FORM.ADD_TITLE') as string;
    userFormState.item = undefined;
    userPageStore.$patch({ visibleCreateModal: true });
};
const clickUpdate = () => {
    userFormState.updateMode = true;
    userFormState.headerTitle = t('IDENTITY.USER.FORM.UPDATE_TITLE') as string;
    userFormState.item = userPageState.users[userPageState.selectedIndices[0]];
    userPageStore.$patch({ visibleUpdateModal: true });
};
const clickDelete = () => {
    modalState.mode = 'delete';
    modalState.title = t('IDENTITY.USER.MAIN.DELETE_MODAL_TITLE') as string;
    modalState.subTitle = t('IDENTITY.USER.MAIN.DELETE_MODAL_DESC', userPageState.selectedIndices.length);
    modalState.themeColor = 'alert';
    userPageStore.$patch({ visibleStatusModal: true });
};
const clickEnable = () => {
    modalState.mode = 'enable';
    modalState.title = t('IDENTITY.USER.MAIN.ENABLE_MODAL_TITLE') as string;
    modalState.subTitle = t('IDENTITY.USER.MAIN.ENABLE_MODAL_DESC', userPageState.selectedIndices.length);
    modalState.themeColor = 'safe';
    userPageStore.$patch({ visibleStatusModal: true });
};
const clickDisable = () => {
    modalState.mode = 'disable';
    modalState.title = t('IDENTITY.USER.MAIN.DISABLE_MODAL_TITLE') as string;
    modalState.subTitle = t('IDENTITY.USER.MAIN.DISABLE_MODAL_DESC', userPageState.selectedIndices.length);
    modalState.themeColor = 'alert';
    userPageStore.$patch({ visibleStatusModal: true });
};
...
const addUser = async (item, roleId) => {
    try {
        ...
        showSuccessMessage(t('IDENTITY.USER.MAIN.ALT_S_ADD_USER'), '');
