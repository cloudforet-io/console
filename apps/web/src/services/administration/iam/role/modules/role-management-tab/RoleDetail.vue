...
const baseInfoState = reactive({
    title: computed(() => t('IAM.ROLE.DETAIL.BASE_INFORMATION')),
    loading: true,
    timezone: computed(() => store.state.user.timezone || 'UTC'),
    fields: computed<DataTableTranslationField[]>(() => [
        { name: 'name', label: t('IAM.ROLE.DETAIL.NAME') },
        { name: 'tags.description', label: t('IAM.ROLE.DETAIL.DESCRIPTION') },
        { name: 'role_type', label: t('IAM.ROLE.DETAIL.ROLE_TYPE'), disableCopy: true },
        { name: 'created_at', label: t('IAM.ROLE.DETAIL.CREATED_AT') },
    ]),
...
const convertPagePermissionLabel = (data) => {
    ...
    case PAGE_PERMISSION_TYPE.MANAGE:
        return t('IAM.ROLE.FORM.MANAGE');
    case PAGE_PERMISSION_TYPE.VIEW:
        return t('IAM.ROLE.FORM.VIEW');
    default:
        return '--';
    }
};
...
<template>
    <div>
        ...
        />
        <p-definition-table :fields="baseInfoState.fields"
                            :data="baseInfoState.data"
