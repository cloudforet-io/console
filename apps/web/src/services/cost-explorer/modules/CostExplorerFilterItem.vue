...
const emit = defineEmits<{(e: 'update-filter-items', value: FilterItem[]): void}>();
...
const handleSelectedProjectIds = (selectedProjectIds) => {
    const updatedFilterItems: FilterItem[] = selectedProjectIds.map((d) => ({
        k: props.category,
        v: d,
        o: '=',
    }));
...
const handleUpdateSelected = (selectedItems: FilterableDropdownMenuItem[]) => {
    const updatedFilterItems: FilterItem[] = selectedItems.map((d) => ({
        k: props.category,
        v: d.name as string,
        o: '=',
    }));
...
const handleUpdateSelectedQueryItems = (selectedQueryItems: QueryItem[]) => {
    const updatedFilterItems: FilterItem[] = selectedQueryItems.map((d) => ({
...
<template>
    <div class="cost-explorer-filter-item">
        <project-select-dropdown v-if="category === FILTER.PROJECT"
                                 multi-selectable
                                 project-selectable
                                 :selected-project-ids="state.selectedItems.map(d => d.name)"
