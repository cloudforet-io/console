...
const state = reactive({
    ...
    cards: computed<Card[]>(() => [
        // total cost
        {
            title: t('BILLING.COST_MANAGEMENT.BUDGET.STAT.TOTAL_COST'),
        ...
        // total budget
        {
            title: t('BILLING.COST_MANAGEMENT.BUDGET.STAT.TOTAL_BUDGET'),
        ...
        // budget usage
        {
            title: t('BILLING.COST_MANAGEMENT.BUDGET.STAT.BUDGET_USAGE'),
    ...
    availableBudget: computed<string>(() => {
        ...
        return `${currencyMoneyFormatter(availableBudget, state.currency, state.currencyRates)} ${t('BILLING.COST_MANAGEMENT.BUDGET.STAT.AVAILABLE')}`;
    }),
    // api request params
...
<template>
    <div class="budget-stat">
        <div v-for="({title, data, unit, description}) in state.cards"
             :key="title"
             class="card-box"
        >
            <div class="title">
            ...
            <template v-else>
                <div class="stat">
                    <template v-if="unit === UNIT.currency">
                        <span>{{ currencyMoneyFormatter(data, state.currency, state.currencyRates) }}</span>
                    </template>
