...
<template>
    <cost-dashboard-simple-card-widget
        ...
        :value="currencyMoneyFormatter(state.currentMonthCost, currency, currencyRates, true, 10000000000)"
        :currency-symbol="state.currencySymbol"
        :description="thisMonthFormatter(state.currentMonth)"
        :no-data="!state.loading && !state.currentMonthCost && !state.lastMonthCost"
        :class="{ 'print-mode': printMode }"
    >
        <template #default>
            <div class="cost-trend-wrapper flex">
                <span class="increase increase-rate"
                      :class="[{safe: state.increaseRate < 0}, {alert: state.increaseRate > 0}]"
                >
                    <p-i v-if="state.increaseRate < 0"
                ...
                <span class="increase increase-amount">
                    ...
                    <p-i v-else
                         ...
                         height="1rem"
                    />
                    <span class="unit">{{ state.currencySymbol }} </span>
                    <span>{{ currencyMoneyFormatter(Math.abs(state.increaseCost), currency, currencyRates, true, 10000000) }}</span>
                </span>
            </div>
            <span class="range">
                <template v-if="state.increaseRate < 0">
                    <span class="range"> {{ t('BILLING.COST_MANAGEMENT.DASHBOARD.DECREASE', { date: thisMonthFormatter(state.lastMonth) }) }} </span>
                </template>
                <template v-else>
                    <span class="range"> {{ t('BILLING.COST_MANAGEMENT.DASHBOARD.INCREASE', { date: thisMonthFormatter(state.lastMonth) }) }} </span>
                </template>
            </span>
        </template>
    </cost-dashboard-simple-card-widget>
