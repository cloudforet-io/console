...
const state = reactive({
    ...
    items: computed<Item>(() => ({
        project: {
            label: t('COMMON.WIDGETS.FAVORITES_WIDGET.LABEL_PROJECT'),
            favorites: state.isExpanded ? state.favoriteProjects : state.favoriteProjects.slice(0, LIMIT_COUNT),
        },
        cloudService: {
            label: t('COMMON.WIDGETS.FAVORITES_WIDGET.LABEL_CLOUD_SERVICE'),
    ...
    <div class="favorites-widget">
        ...
        <div class="list-wrapper">
            <div class="label-wrapper">
                <label v-for="(item, k) in Object.values(state.items)"
                       :key="k"
                >{{ item.label }}</label>
            </div>
            <div class="item-wrapper">
                <div v-for="(item, k) in Object.values(state.items)"
                     :key="k"
        ...
        <summary v-if="state.showToggle"
                 class="toggle-btn"
                 @click="handleClickToggle"
        >
            {{ state.isExpanded ? t('COMMON.WIDGETS.FAVORITES_WIDGET.TOGGLE_LESS') : t('COMMON.WIDGETS.FAVORITES_WIDGET.TOGGLE_MORE') }}
