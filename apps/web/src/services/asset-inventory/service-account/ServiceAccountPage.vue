...
const queryHelper = new QueryHelper().setFiltersAsRawQueryString(query.filters as undefined|string|(string|null)[]);
...
const fetchOptionState = reactive({
    ...
    sortBy: 'created_at',
    // TODO: need to implement type assertion
    queryTags: queryHelper.setFiltersAsRawQueryString(query.filters as undefined|string|(string|null)[]).queryTags,
});
...
<template>
    <section class="service-account-page">
        ...
        />
        <provider-list v-model:selected-provider="state.selectedProvider"
