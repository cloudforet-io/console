...
const handleClickTimeline = (item: CloudServiceHistoryItem) => {
    ...
    router.push({ hash: HISTORY_OVERLAY_HASH_NAME });
};
...
<template>
    <div class="cloud-service-history">
        <p-heading heading-type="sub"
                   ...
                   use-total-count
        />
        <p-toolbox v-model:search-text="state.searchText"
                   search-type="query"
                   searchable
        ...
        >
            <template #left-area>
                <cloud-service-history-date-select-dropdown v-model:selected-year="state.selectedYear"
                                                            v-model:selected-month="state.selectedMonth"
                />
            </template>
        </p-toolbox>
        <p-data-loader :data="state.items"
                       :loading="!state.items.length && state.loading"
        >
            <div ref="timelineWrapperRef"
                 class="timeline-wrapper"
            >
                <vertical-timeline v-for="(item, idx) in state.items"
                                   :key="`timeline-${item.date}-${idx}`"
                                   :date="item.date"
                                   :title="item.title"
                                   :count="item.diffCount"
                                   :color="getTimelineColor(item.action)"
                                   :is-last-item="idx === state.items.length - 1"
                ...
                <p-spinner v-if="state.loading && !!state.items.length"
                           size="xl"
                />
            </div>
        </p-data-loader>
        <transition name="slide-up">
            <cloud-service-history-detail-overlay v-if="state.showDetailOverlay"
                                                  v-model:selected-history-item="state.selectedHistoryItem"
                                                  :loading="state.loading"
                                                  :history-items="state.items"
