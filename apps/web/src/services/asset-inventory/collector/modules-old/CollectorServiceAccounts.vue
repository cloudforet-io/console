...
const openCollectDataModal = (item: SecretModel) => {
    ...
    <div class="collector-credentials">
        ...
        >
            ...
            <template #col-created_at-format="{value}">
                <span>{{ iso8601Formatter(value, state.timezone) }}</span>
            </template>
        ...
        <collect-data-modal v-if="state.collectDataVisible"
                            v-model:visible="state.collectDataVisible"
                            :collector-id="collectorId"
                            :credential-id="state.targetCredentialId"
        />
    </div>
</template>
