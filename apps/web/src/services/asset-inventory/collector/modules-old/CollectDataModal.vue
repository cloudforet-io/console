...
const onClickCollectConfirm = async (): Promise<void> => {
    ...
    try {
        ...
        showSuccessMessage(t('PLUGIN.COLLECTOR.MAIN.ALT_S_COLLECT_START_TITLE'), '');
    } catch (e) {
        ErrorHandler.handleRequestError(e, t('PLUGIN.COLLECTOR.MAIN.ALT_E_COLLECT_TITLE'));
    } finally {
...
<template>
    <p-button-modal v-model:visible="state.proxyVisible"
                    :header-title="t('PLUGIN.COLLECTOR.MAIN.COLLECT_MODAL_TITLE')"
                    centered
                    size="sm"
                    fade
                    backdrop
        ...
        <template #body>
            <div class="w-full">
                <div class="flex w-full">
                    ...
                    <div>
                        <p class="name">
                            {{ state.collector ? state.collector?.name : '' }}
                        </p>
                        <p class="info">
                            {{ t('PLUGIN.COLLECTOR.MAIN.COLLECT_MODAL_LABEL_ID') }}: {{ collectorId }}
                        </p>
                        <p class="info">
                            {{ t('PLUGIN.COLLECTOR.MAIN.COLLECT_MODAL_LABEL_PLUGIN') }}: {{ state.pluginName }}
                        </p>
                        <p class="info">
                            {{ t('PLUGIN.COLLECTOR.MAIN.COLLECT_MODAL_LABEL_VERSION') }}: {{ state.version }}
                        </p>
                    </div>
                </div>
                <p class="desc">
                ...
                <table class="w-full">
                    <p-definition
                        ...
                        :data="state.credential ? state.credential.name : t('PLUGIN.COLLECTOR.MAIN.COLLECT_MODAL_CREDENTIALS_ALL')"
                        auto-key-width
                    />
                </table>
            </div>
