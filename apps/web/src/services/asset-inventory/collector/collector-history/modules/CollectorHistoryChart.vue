...
import {
    reactive, watch, computed, onUnmounted,
} from 'vue';
...
const emit = defineEmits<{(e: 'click-date', value: { type: ValueType, date: any }): void}>();
const { t } = useI18n();
...
const state = reactive({
    ...
    legends: computed(() => ([
        {
            label: t('MANAGEMENT.COLLECTOR_HISTORY.MAIN.COMPLETED'),
            color: green[400],
        },
        {
            label: t('MANAGEMENT.COLLECTOR_HISTORY.MAIN.FAILED'),
            color: coral[400],
        },
    ])),
    //
