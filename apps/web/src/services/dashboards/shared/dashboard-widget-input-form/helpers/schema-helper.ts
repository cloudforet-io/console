...
export const getRefinedWidgetOptionsSchema = (
    ...
    const refinedJsonSchema: Required<Pick<WidgetOptionsSchema['schema'], 'type'|'properties'|'required'|'order'>> = {
        type: 'object',
        properties: {},
...
export const getInheritOptionsForProjectDashboard = (inheritOptions: InheritOptions|undefined, projectId: string): InheritOptions => {
    let _inheritOptions = inheritOptions ?? {};
    if (!projectId) return _inheritOptions ?? {};
    _inheritOptions = {
        ..._inheritOptions,
