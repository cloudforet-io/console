...
import {
    getInheritOptionsForProjectDashboard,
...
const state = reactive({
    widgetInfo: computed<DashboardLayoutWidgetInfo|undefined>(() => (props.widgetKey ? widgetFormStore.initWidgetForm(props.widgetKey) : undefined)),
...
const mergedState = useMergedWidgetState({
    inheritOptions: computed(() => state.widgetInfo?.inherit_options),
    widgetOptions: computed(() => state.widgetInfo?.widget_options),
    widgetName: computed(() => state.widgetInfo?.widget_name),
...
const getFormDataFromMergedWidgetData = (merged: UnwrapRef<MergedWidgetState>) => {
    const formData = {};

    // set schema keywords
    Object.entries(merged.options).forEach(([optionKey, optionValue]) => {
    ...
    Object.entries(merged.inheritOptions).forEach(([optionKey, optionValue]) => {
...
const getDefaultValueForSchemaForm = (optionKey: string): any => {
    const filterKey = optionKey.replace('filters.', '');

    // inherit value from variable
    if (mergedState.inheritOptions[optionKey]?.enabled) {
        if (dashboardDetailState.variablesSchema.properties[filterKey]?.use) {
            return filterKey;
        }
        return undefined;
    }

    // get default value from widget option schema
    return state.widgetOptionsJsonSchema.properties?.[optionKey]?.default;
};
...
const initStatesByWidgetConfig = (widgetConfigId: string, forceInit = false) => {
    ...
    widgetFormStore.$patch((_state) => {
        ...
        _state.inheritOptions = mergedState.inheritOptions;
    });
    if (!props.widgetKey || forceInit) {
        // set default values to fixed properties
        widgetFormState.schemaProperties?.forEach((propertyName) => {
            state.schemaFormData[propertyName] = getDefaultValueForSchemaForm(propertyName);
        });
    }
    // init states
    state.widgetOptionsJsonSchema = getRefinedWidgetOptionsSchema(
...
const setStatesForEditMode = (widgetKey: string) => {
    ...
    widgetFormStore.$patch((_state) => {
        if (dashboardDetailState.projectId) {
            _state.inheritOptions = getInheritOptionsForProjectDashboard(mergedState.inheritOptions, dashboardDetailState.projectId);
        }
    ...
    // init form data
    state.schemaFormData = getFormDataFromMergedWidgetData(mergedState);
};
