...
const state = reactive({
    ...
    leftBudget: computed<{label: string, color?: string}|undefined>(() => {
        ...
        if (value >= 0) {
            return {
                label: `${currencyMoneyFormatter(value, widgetState.currency)} ${t('DASHBOARDS.WIDGET.BUDGET_USAGE_SUMMARY.AVAILABLE')}`,
            };
        }
        return {
            label: `${currencyMoneyFormatter(Math.abs(value), widgetState.currency)} ${t('DASHBOARDS.WIDGET.BUDGET_USAGE_SUMMARY.EXCEEDED')}`,
            color: red[400],
        };
    }),
