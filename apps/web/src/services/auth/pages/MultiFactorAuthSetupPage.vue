...
const state = reactive({
    ...
    needToEnableMFA: computed<boolean>(() => {
        ...
        const needToEnableMFA = isMFAEnforced && store.state.mfa?.state !== 'ENABLED';
...
/* API */
const confirmMfa = async (params: UserProfileConfirmEmailParameters) => {
    state.isConfirmMfaPending = true;
    try {
        ...
        state.isConfirmMfaPending = false;
    }
};
...
<template>
    <div class="multi-factor-authentication-page">
        ...
        >
            ...
            <p-button size="lg"
                      :loading="state.isConfirmMfaPending"
