...
const createProjectGroup = async (item) => {
    try {
        ...
        showSuccessMessage(t('PROJECT.LANDING.ALT_S_CREATE_PROJECT_GROUP'), '');
    } catch (e) {
        ErrorHandler.handleRequestError(e, t('PROJECT.LANDING.ALT_E_CREATE_PROJECT_GROUP'));
    }
};
...
<template>
    <p-button-modal v-model:visible="state.proxyVisible"
                    :header-title="projectPageStore.projectGroupFormUpdateMode ? t('PROJECT.LANDING.MODAL_UPDATE_PROJECT_GROUP_TITLE') : t('PROJECT.LANDING.MODAL_CREATE_PROJECT_GROUP_TITLE')"
                    centered
                    size="sm"
                    fade
                    backdrop
        ...
        <template #body>
            ...
            >
                <template #default="{invalid}">
                    <p-text-input v-model:value="state.projectGroupName"
                                  class="block w-full"
