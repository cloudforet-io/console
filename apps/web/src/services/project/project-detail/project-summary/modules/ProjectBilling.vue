...
const state = reactive({
    ...
    skeletons: range(4),
    dateTypes: computed(() => ([
        { name: DATE_TYPE.daily, label: t('COMMON.WIDGETS.BILLING.DAY') },
        { name: DATE_TYPE.monthly, label: t('COMMON.WIDGETS.BILLING.MONTH') },
    ])),
    data: [],
...
const summaryState = reactive({
    pastDateText: computed(() => {
        if (state.selectedDateType === DATE_TYPE.monthly) return t('COMMON.WIDGETS.BILLING.LAST_MONTH');
        return t('COMMON.WIDGETS.BILLING.TWO_DAYS_AGO');
    }),
    currentDateText: computed(() => {
        if (state.selectedDateType === DATE_TYPE.monthly) return t('COMMON.WIDGETS.BILLING.THIS_MONTH');
        return t('COMMON.WIDGETS.BILLING.YESTERDAY');
    }),
    pastDate: computed(() => {
...
const drawChart = (_chartContext) => {
    const createChart = () => {
        ...
        chartState.registry[state.chartRef] = am4core.create(_chartContext, XYChart);
    ...
    <div class="project-billing">
        ...
        <div class="content-wrapper grid grid-cols-12 gap-2">
            ...
            <div class="col-span-12 detail-wrapper">
                ...
                >
                    <p-collapsible-toggle v-model:is-collapsed="tableState.isCollapsed">
