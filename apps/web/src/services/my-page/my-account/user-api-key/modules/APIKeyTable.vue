    ...
    dropdownMenu: computed(() => ([
        {
            type: 'item',
            name: 'enable',
            label: t('IDENTITY.USER.MAIN.ENABLE'),
        ...
        { type: 'divider' },
        {
            type: 'item',
            name: 'disable',
            label: t('IDENTITY.USER.MAIN.DISABLE'),
            disabled: state.selectedIndex.length !== 1 || state.selectedItems[0].state === 'DISABLED',
        },
        { type: 'divider' },
        {
            type: 'item', name: 'delete', label: t('IDENTITY.USER.MAIN.DELETE'), disabled: state.selectedIndex.length !== 1,
        },
    ] as MenuItem[])),
    visible: false,
    user: props.userId || '',
...
const openAPIKeyConfirmModal = async () => {
    try {
        ...
        ErrorHandler.handleRequestError(e, t('IDENTITY.USER.API_KEY.ALT_E_CREATE_SCHEDULER'));
    } finally {
...
const enableAPIKey = async (item) => {
    try {
        ...
        showSuccessMessage(t('IDENTITY.USER.MAIN.ALT_S_ENABLE_API_KEY'), '');
    } catch (e) {
        ErrorHandler.handleRequestError(e, t('IDENTITY.USER.MAIN.ALT_S_ENABLE_API_KEY'));
    } finally {
        await listAPIKey(state.user);
...
const disableAPIKey = async (item) => {
    try {
        ...
        showSuccessMessage(t('IDENTITY.USER.MAIN.ALT_S_DISABLE_API_KEY'), '');
    } catch (e) {
        ErrorHandler.handleRequestError(e, t('IDENTITY.USER.MAIN.ALT_E_DISABLE_API_KEY'));
    } finally {
        await listAPIKey(state.user);
        checkModalState.visible = false;
    }
...
const deleteAPIKey = async (item) => {
    try {
        ...
        showSuccessMessage(t('IDENTITY.USER.MAIN.ALT_S_DELETE_API_KEY'), '');
    } catch (e) {
        ErrorHandler.handleRequestError(e, t('IDENTITY.USER.MAIN.ALT_E_DELETE_API_KEY'));
    } finally {
        state.selectedIndex = [];
        await listAPIKey(state.user);
        checkModalState.visible = false;
...
const onClickEnable = async () => {
    ...
    checkModalState.title = t('IDENTITY.USER.API_KEY.ENABLE_MODAL_TITLE') as string;
    checkModalState.subTitle = t('IDENTITY.USER.API_KEY.ENABLE_MODAL_DESC', state.selectedIndex.length);
...
const onClickDisable = async () => {
    ...
    checkModalState.title = t('IDENTITY.USER.API_KEY.DISABLE_MODAL_TITLE') as string;
    checkModalState.subTitle = t('IDENTITY.USER.API_KEY.DISABLE_MODAL_DESC', state.selectedIndex.length);
...
const onClickDelete = async () => {
    ...
    checkModalState.title = t('IDENTITY.USER.API_KEY.DELETE_MODAL_TITLE') as string;
    checkModalState.subTitle = t('IDENTITY.USER.API_KEY.DELETE_MODAL_DESC', state.selectedIndex.length);
    checkModalState.themeColor = 'alert';
    checkModalState.visible = true;
};
...
<template>
    <section class="right-contents-container">
        <p-pane-layout class="main-table-wrapper">
            ...
            <p-data-table
                v-model:select-index="state.selectedIndex"
                :items="state.items"
        ...
        <user-a-p-i-key-modal v-if="state.visible && !modalState.loading"
