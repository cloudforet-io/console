...
const escalationPolicyApiQueryHelper = new ApiQueryHelper()
    ...
    .setFiltersAsRawQueryString(currentQuery.filters as string);
...
const state = reactive({
    ...
    pageTitle: computed(() => t('MONITORING.ALERT.ESCALATION_POLICY.ESCALATION_POLICY')),
    actionItems: computed(() => ([
        {
            type: 'item',
            name: 'update',
            label: t('MONITORING.ALERT.ESCALATION_POLICY.UPDATE'),
        },
        {
            type: 'item',
            name: 'delete',
            label: t('MONITORING.ALERT.ESCALATION_POLICY.DELETE'),
            disabled: state.selectedItem?.is_default,
        },
        {
            type: 'item',
            name: 'default',
            label: t('MONITORING.ALERT.ESCALATION_POLICY.SET_AS_DEFAULT'),
...
const setDefaultEscalationPolicy = async () => {
    try {
        ...
        showSuccessMessage(t('MONITORING.ALERT.ESCALATION_POLICY.ALT_S_SET_AS_DEFAULT'), '');
        await listEscalationPolicies();
    } catch (e) {
        ErrorHandler.handleRequestError(e, t('MONITORING.ALERT.ESCALATION_POLICY.ALT_E_SET_AS_DEFAULT'));
    }
};
const deleteEscalationPolicy = async () => {
    try {
        ...
        showSuccessMessage(t('MONITORING.ALERT.ESCALATION_POLICY.ALT_S_DELETE_POLICY'), '');
        state.selectIndex = [];
        await listEscalationPolicies();
    } catch (e) {
        ErrorHandler.handleRequestError(e, t('MONITORING.ALERT.ESCALATION_POLICY.ALT_E_DELETE_POLICY'));
    } finally {
...
<template>
    <div class="escalation-policy-page">
        ...
        <escalation-policy-form-modal
            v-model:visible="state.formModalVisible"
            :mode="state.formMode"
