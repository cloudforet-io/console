...
export const convertDashboardConfigToReferenceData = (config: ConfigData[]|null, dashboardList): ReferenceData[] => {
    ...
    dashboardList.forEach((dashboard, index) => {
        dashboardMap[dashboard.dashboard_id || dashboard.private_dashboard_id] = index;
    });
    ...
    config.forEach((d) => {
        ...
        if (resource) {
            const index = dashboardMap[resource.dashboard_id || resource.private_dashboard_id || ''];
            results[index] = {
                ...d,
                name: resource.dashboard_id || resource.private_dashboard_id,
                label: resource.name,
