...
import {
    CategoryAxis, ValueAxis, LineSeries, XYCursor, XYChart,
...
const drawChart = (ctx) => {
    const chart: any = create(ctx, XYChart);
    ...
    dateAxis.renderer.grid.template.stroke = color(gray[200]);
    ...
    if (state.data.length > 0) {
        ...
        chart.cursor.lineX.stroke = color(gray[900]);
...
watch([() => chartContext.value, () => props.loading], async ([ctx, loading]) => {
    if (ctx && !loading) {
