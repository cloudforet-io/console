...
const legendFormatter = (resource) => (resource.name ? `${resource.name} (${resource.id})` : `(${resource.id})`);
...
<template>
    <div class="monitoring">
        <section v-if="!dataSourceId && state.dataTools.length > 1"
                 class="data-source-section"
        >
            <p-select-button-group v-model:selected="state.selectedToolId"
        ...
        <section class="chart-section">
            ...
            <template v-else>
                <div class="metric-chart-wrapper grid grid-cols-12">
                    <metric-chart v-for="(item, index) in state.metricChartDataList"
                                  :key="index"
