...
<template>
    <p-button-modal v-model:visible="state.visible"
                    class="survey-modal"
                    ...
                    :disabled="!state.selectedAnswer1 || !state.selectedAnswer2"
            ...
            <div class="content-wrapper">
                <div class="question-wrapper">
                    ...
                    <div class="select-card-wrapper">
                        <p-select-card v-for="item in state.answerItems1"
                                       :key="`answer1-${item.name}`"
                                       v-model:selected="state.selectedAnswer1"
                                       :image-url="require(`@/assets/images/${item.name === state.selectedAnswer1 ? item.selectedImage : item.unselectedImage}.svg`)"
                                       icon="ic_webhook"
                                       :value="item.name"
                                       :label="item.label"
                        />
                ...
                <div class="question-wrapper">
                    ...
                    <div class="select-card-wrapper">
                        <p-select-card v-for="(item, index) in state.answerItems2"
                                       :key="`answer2-${item.name}`"
                                       v-model:selected="state.selectedAnswer2"
                                       :tab-index="index"
                                       :image-url="require(`@/assets/images/${item.name === state.selectedAnswer2 ? item.selectedImage : item.unselectedImage}.svg`)"
                                       icon="ic_webhook"
                                       :value="item.name"
                                       :label="item.label"
                        />
