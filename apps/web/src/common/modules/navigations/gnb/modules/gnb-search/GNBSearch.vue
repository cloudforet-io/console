...
const handleSelect = (index: number, type: SuggestionType) => {
    if (type === 'MENU') {
        ...
        if (menuInfo && router.currentRoute.value.name !== menuId) {
            router.push({ name: menuId }).catch(() => {});
...
<template>
    ...
    >
        <g-n-b-search-input v-if="state.isOverLaptopSize"
                            ...
                            v-model:is-focused="state.isFocusOnInput"
        ...
        <g-n-b-search-dropdown v-show="visible"
                               v-model:focusing-direction="state.focusingDirection"
                               v-model:is-focused="state.isFocusOnSuggestion"
