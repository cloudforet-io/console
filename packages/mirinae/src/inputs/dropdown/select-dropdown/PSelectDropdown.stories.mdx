...
Currently, 'selected' has the proxy applied automatically.
If you want to manipulate 'selected' directly on the page, set 'disableProxy' to true.

<Canvas>
    <Story name="Disable Proxy" args={{menu: getSelectDropdownMenuWithMultiTypes()}} height={'400px'}>
        {{
            props: Object.keys(getSelectDropdownArgTypes()),
            components: { PSelectDropdown, PButton },
            template: `
            <div>
                <p class="text-label-lg font-bold my-3">disableProxy: false</p>
                <p-select-dropdown :menu="menu" />
                <br/>
                <p class="text-label-lg font-bold my-3">disableProxy: true</p>
                <p-select-dropdown :menu="menu" disable-proxy />
                <br/>
            </div>
`,
        }}
