...
<Canvas>
    <Story name="Basic" >
        {{
            components: { PTextInput },
            template: `
                <p-text-input v-model:value="value" />
            `,
            setup() {
        ...
        {{
            ...
            template: `
                <div style="width: 90%">
                    ...
                    <p-text-input :menu="menu" v-model:selected="selected" use-auto-complete use-fixed-menu-style />
                    <br/>
                    <br/>
                    <p class="text-label-lg font-bold my-3">With multi input</p>
                    <p-text-input :menu="menu" v-model:selected="selected" multi-input use-auto-complete use-fixed-menu-style />
                    <br/>
                    <br/>
                    <div class="flex mt-4">
...
<Canvas>
    <Story name="Multi Input Duplicate Checks">
        {{
            ...
            template: `
                <div style="width: 80%">
                    <p-text-input v-model:selected="selected" multi-input use-fixed-menu-style @update="handleUpdate" />
                    <div class="flex mt-4">
        ...
        {{
            ...
            template: `
                <div>
                    ...
                    <p-text-input :menu="menu" v-model:selected="selected" use-auto-complete :page-size="5" use-fixed-menu-style />
                    <br/>
                    <br/>
                    <p class="text-label-lg font-bold my-3">With multi input, page size 5</p>
                    <p-text-input :menu="menu" v-model:selected="selected" use-auto-complete multi-input :page-size="5" use-fixed-menu-style />
                    <br/>
                    <br/>
                    <p class="text-label-lg font-bold my-3">With custom handler, page size 5</p>
                    <p-text-input v-model:selected="selected" :handler="simpleHandler" use-auto-complete :page-size="5" use-fixed-menu-style />
                    <br/>
                    <br/>
                    <p class="text-label-lg font-bold my-3">With custom handler, multi input, page size 5</p>
                    <p-text-input v-model:selected="selected" :handler="simpleHandler" use-auto-complete multi-input :page-size="5" use-fixed-menu-style />
                    <br/>
                    <br/>
                    <div class="p-4 bg-blue-200 w-full max-h-64 overflow-y-auto">
                        <p class="font-lg font-bold">selected: </p>
...
<Canvas>
    <Story name="Placeholder" >
        {{
            components: { PTextInput },
            template: `
                <p-text-input v-model:value="value" class="test" placeholder="Placeholder" />
            `,
            setup() {
                const state = reactive({
                    value: ''
...
<Canvas>
    <Story name="Number Type">
        {{
            components: { PTextInput },
            template: `
                <p-text-input v-model:value="value" type="number" :min="0" :max="100" />
            `,
            setup() {
                const state = reactive({
                    value: 99
...
<Canvas>
    <Story name="Disabled">
        {{
            components: { PTextInput },
            template: `
                <p-text-input v-model:value="value" disabled />
            `,
            setup() {
                const state = reactive({
                    value: 'value'
...
<Canvas>
    <Story name="Block & Multi Input">
        {{
            ...
            template: `
                <div>
                    <p-text-input v-model:selected="entered" block multi-input />
...
<Canvas>
    <Story name="Invalid">
        {{
            components: { PTextInput },
            template: `
                <p-text-input v-model:value="value" invalid />
            `,
            setup() {
                const state = reactive({
                    value: 'value'
        ...
        {{
            ...
            template: `
                <form>
                    ...
                    <br/><br/>
                    <p-text-input v-model:value="id" placeholder="ID" autocomplete="username" />
                    <br/><br/>
                    <p-text-input v-model:value="pw" type="password" placeholder="PW" autocomplete="current-password" />
                </form>
            `,
            setup() {
                const state = reactive({
...
<p-text-input type="number" multi-input v-model:value="value" >
    <template #input-right>%</template>
</p-text-input>
<p class="text-lg font-bold my-4">right-edge slot</p>
<p-text-input multi-input v-model:value="value2" >
        ...
        {{
            ...
            template: `
                <div  style="display:flex; flex-direction: column; row-gap: 1.5rem;">
                    <p-text-input v-for="size in sizes" :key="size" :size="size" v-model:value="value" :placeholder="size" />
                </div>
            `,
            setup() {
                const state = reactive({
